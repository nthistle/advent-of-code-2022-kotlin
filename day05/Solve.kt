...
fun main() {
    val (initialCrates, rawOperations) = File("input.txt").readText().trim().split("\n\n").map { it.lines() }
    val crateConfig = initialCrates.dropLast(1).map {
        it.chunked(4).map { it[1] }
    }.let { crates ->
        val n = crates.last().size
        (0..(n-1)).map { i -> crates.map { it[i] }.filter { it != ' ' }.reversed() }
    }
    val operations = rawOperations.map {
        val (count, src, dst) = nums(it).toList()
        Operation(count, src - 1, dst - 1)
    }

    println(crateConfig.map { it.toMutableList() } .let {  crates ->
        operations.forEach {
            operation -> 
